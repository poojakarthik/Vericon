...
<div id="users-contain" class="ui-widget">
	<table id="users" class="ui-widget ui-widget-content">
		...
		if ($query == "")
		{
			$q = mysql_query("SELECT * FROM auth WHERE type LIKE '%tpv%' LIMIT 0,10")
			  or die(mysql_error());
		...
		{	
			$q = mysql_query("SELECT * FROM auth WHERE " . $method . " LIKE '%" . mysql_escape_string($query) . "%' AND type LIKE '%tpv%' ORDER BY user ASC")
			  or die(mysql_error());
