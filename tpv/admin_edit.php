...
<div id="dialog-form" title="Submit Changes">
<?php
$qs = mysql_query("SELECT status FROM sales_customers WHERE id = '$_GET[id]'") or die(mysql_error());
$status = mysql_fetch_row($qs);
...
<option <?php echo $ap; ?>>Approved</option>
<option <?php echo $de; ?>>Declined</option>
<option <?php echo $li; ?>>Line Issue</option>
<option <?php echo $ho; ?>>Hold</option>
</select></td>
</tr>
<tr>
<td width="50px">Reason </td>
<td><textarea id="notes" style="width:350px; height:100px; resize:none;"></textarea></td>
</tr>
</table>
</div>
...
if ($_GET["id"] == "")
{
	echo "<script>window.location = '../tpv/admin.php?p=search';</script>";
}
else
{
	$id = $_GET["id"];
...
window.onload=function()
{
	...
	var id = $( "#physical" );
	$( '#physical_address' ).load('address.php?id=' + id.val());
	...
	{
		var id = $( "#postal" );
		$( '#postal_address' ).load('address.php?id=' + id.val());
		if ( id.val().substring(0,2) == "GA" )
		{
			$.get("../source/gnafGet.php?type=display&gnaf_id=" + id.val(), {  },
				function(data) {
