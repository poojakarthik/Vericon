...
{
	id = "<?php echo $_GET["id"]; ?>";
	$.get("details_submit.php" , { method: "notes", id: id }, function(data) {
		$( "#notes" ).val(data);
	});
	...
	$( "#dialog-form2" ).dialog({
		...
		buttons: {
			"Add Package": function() {
				...
				$.get("details_submit.php?method=add_nz", { id: id, cli: cli.val(), plan: plan.val(), plan_type: plan_type.val(), provider: provider.val(), ac_number: ac_number.val(), adsl_provider: adsl_provider.val(), adsl_ac_number: adsl_ac_number.val() },
				function(data) {
					if (data == "added")
					{
...
function Plan_Dropdown()
{
	...
	$( "#plan" ).val("");
	$( "#plan" ).load("plans_nz.php?option=add&id=<?php echo $_GET["id"]; ?>&type=" + $( "#sale_type" ).val() + "&cli=" + $('#cli').val(), function() {
	...
	$( "#dialog-form3" ).dialog({
		...
		buttons: {
			"Edit Package": function() {
				...
				$.get("details_submit.php?method=edit_nz", { id: id, cli: cli.val(), plan: plan.val(), plan_type: plan_type.val(), provider: provider.val(), ac_number: ac_number.val(), adsl_provider: adsl_provider.val(), adsl_ac_number: adsl_ac_number.val(), cli2: cli2.val() },
				function(data) {
					if (data == "editted")
					{
						$( "#packages" ).load('packages_nz.php?id=' + id);
...
function Edit_Package(cli,plan)
{
	...
	$( "#edit_plan" ).load("plans_nz.php?option=edit&id=<?php echo $_GET["id"]; ?>&type=" + $( "#sale_type" ).val() + "&cli=" + $('#edit_cli').val(), function() {
		$( "#edit_cli" ).removeAttr("disabled");
		$( "#edit_plan" ).removeAttr("disabled");
		$( "#edit_plan" ).val(plan);
	});
	$.get("details_submit.php?method=nz_provider", { id: "<?php echo $_GET["id"]; ?>", cli: cli }, function(data) { $( "#edit_provider" ).val(data); });
	$.get("details_submit.php?method=nz_ac_number", { id: "<?php echo $_GET["id"]; ?>", cli: cli }, function(data) { $( "#edit_ac_number" ).val(data); });
	$.get("details_submit.php?method=nz_adsl_provider", { id: "<?php echo $_GET["id"]; ?>", cli: cli }, function(data) {
		if (data != "")
		{
	...
	$.get("details_submit.php?method=nz_adsl_ac_number", { id: "<?php echo $_GET["id"]; ?>", cli: cli }, function(data) {
		if (data != "")
		{
			$( "#edit_plan_type" ).val("Bundle");
...
function Delete_Package(cli)
{
	...
	$.get("details_submit.php?method=delete", { id: id, cli: cli},
	function(data) {
		if (data == "deleted")
		{
			$( "#packages" ).load('packages_nz.php?id=' + id);
...
function Done()
{
	...
	$.get("details_submit.php?method=submit_nz", { id: id, title: title.val(), first: first.val(), middle: middle.val(), last: last.val(), dob: dob.val(), email: email.val(), mobile: mobile.val(), physical: physical.val(), postal: postal.val(), id_type: id_type.val(), id_num: id_num.val(), bus_name: bus_name.val(), position: position.val(), ongoing_credit: ongoing_credit.val(), onceoff_credit: onceoff_credit.val() },
	function(data) {
		if (data == "submitted")
		{
...
<select id="edit_plan" style="margin-left:0px; width:210px;">
<option></option>
</select></td>
</tr>
<tr>
...
<table width="100%">
<tr>
<td width="115px">Once Off Credit ($) </td>
