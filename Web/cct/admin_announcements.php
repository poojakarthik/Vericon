...
function Edit(id,subject,message)
{
	$( "#e_id" ).val(id);
	$( "#e_subject" ).val(subject);
	$( "#e_message" ).val(message);
...
<p><img src="../images/cct_announcements_heading.png" width="210" height="25" /></p>
...
<div id="users-contain" class="ui-widget">
	<table id="users" class="ui-widget ui-widget-content">
		...
		<tbody>
        <?php
		$q = mysql_query("SELECT * FROM announcements WHERE department = 'cct' ORDER BY id DESC")
			  or die(mysql_error());

		if (mysql_num_rows($q) == 0)
		{
			echo "<tr>";
			echo "<td colspan='3' style=\"text-align:center\">No Announcements!</td>";
			echo "</tr>";
		}
		else
		{	
			while($r = mysql_fetch_assoc($q))
			{
				...
				echo "<td><a onclick='Edit(\"$r[id]\",\"$r[subject]\",\"" . str_replace("\n",'\n',$r[message]) . "\")' style='cursor:pointer; text-decoration:underline;'>Edit</a></td>";
				if($r["display"] == "Yes")
				{
