...
if ($method == "line")
{
	$q = mysql_query("SELECT sales_customers.id, sales_customers.status, sales_customers.lead_id, sales_customers.agent, sales_customers.campaign, sales_customers.type FROM vericon.sales_customers, vericon.sales_packages WHERE sales_packages.cli = '" . mysql_real_escape_string($query) . "' AND sales_customers.centre = '$centre' AND sales_customers.id = sales_packages.sid ORDER BY sales_customers.id DESC") or die(mysql_error());
}
elseif ($method == "id")
{
	$q = mysql_query("SELECT id,status,lead_id,agent,campaign,type FROM vericon.sales_customers WHERE id = '" . mysql_real_escape_string($query) . "' AND sales_customers.centre = '$centre'") or die(mysql_error());
}
elseif ($method == "lead")
{
	$q = mysql_query("SELECT id,status,lead_id,agent,campaign,type FROM vericon.sales_customers WHERE lead_id = '" . mysql_real_escape_string($query) . "' AND sales_customers.centre = '$centre'") or die(mysql_error());
}
?>
