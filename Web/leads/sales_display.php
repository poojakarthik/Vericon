...
for ($i = 0; $i < count($centres); $i++)
{
	if ($captive[$centres[$i]] == 1)
	{
		...
		$q = mysql_query("SELECT status,type,COUNT(id) FROM vericon.sales_customers WHERE centre = '$centres[$i]' AND DATE(approved_timestamp) = '$date' GROUP BY status,type");
...
<?php
$q = mysql_query("SELECT id FROM vericon.sales_customers WHERE DATE(approved_timestamp) = '$date' AND status = 'Approved'") or die(mysql_error());
