...
$method = $_GET["method"];
$query = $_GET["query"];
$centres = explode(",", $_GET["centre"]);
$centres_query = "";

foreach ($centres as $row)
{
	$centres_query .= "sales_customers.centre = '$row' OR ";
}
$centres_query = substr($centres_query,0,-4);

if ($method == "line")
{
	$q = mysql_query("SELECT sales_customers.id, sales_customers.status, sales_customers.centre, sales_customers.agent, sales_customers.campaign, sales_customers.type FROM vericon.sales_customers, vericon.sales_packages WHERE sales_packages.cli = '" . mysql_real_escape_string($query) . "' AND (" . $centres_query . ") AND sales_customers.id = sales_packages.sid ORDER BY sales_customers.id DESC") or die(mysql_error());
}
elseif ($method == "id")
{
	$q = mysql_query("SELECT id,status,centre,agent,campaign,type FROM vericon.sales_customers WHERE id = '" . mysql_real_escape_string($query) . "' AND (" . $centres_query . ")") or die(mysql_error());
}
elseif ($method == "lead")
{
	$q = mysql_query("SELECT id,status,centre,agent,campaign,type FROM vericon.sales_customers WHERE lead_id = '" . mysql_real_escape_string($query) . "' AND (" . $centres_query . ")") or die(mysql_error());
}
?>
...
{
	echo "<tr>";
	echo "<td colspan='7' style='text-align:center;'>No Results Found</td>";
	echo "</tr>";
}
else
{
