...
if ($method == "create")
{
	...
	else
	{
		...
		$q1 = mysql_query("SELECT pages FROM vericon.portals_template WHERE user = 'TPV'") or die(mysql_error());
...
elseif ($method == "search")
{
	...
	$q = mysql_query("SELECT * FROM vericon.auth WHERE type LIKE '%TPV%' AND (first LIKE '" . mysql_real_escape_string($term[0]) . "%' AND last LIKE '" . mysql_real_escape_string($term[1]) . "%')") or die(mysql_error());
	echo '[';
