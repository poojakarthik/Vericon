...
elseif ($method == "search")
{
	...
	$q = mysql_query("SELECT * FROM vericon.auth WHERE type LIKE '%QA%' AND (first LIKE '" . mysql_real_escape_string($term[0]) . "%' AND last LIKE '" . mysql_real_escape_string($term[1]) . "%')") or die(mysql_error());
	echo '[';
