...
if ($p == "announcements")
{
	if ($method == "post")
	{
		...
		{
			mysql_query("INSERT INTO `announcements` (`date`, `poster`, `department`, `display`, `subject`, `message`) VALUES ('$date', '$poster', 'cs', 'Yes', '" . mysql_escape_string($subject) . "', '" . mysql_escape_string($message) . "');");
			echo "posted";
			exit;
		}
	}
...
elseif ($p == "users")
{
	...
	if ($method == "create")
	{
		...
		else
		{
			...
			mysql_query("INSERT INTO `auth` (`user`,`pass`,`type`,`access`,`status`,`first`,`last`,`alias`) VALUES ('$username','" . md5($password) . "','$type','$access','Enabled','" . mysql_escape_string($first) . "','" . mysql_escape_string($last) . "','" . mysql_escape_string($alias) . "')");
