...
$(function() {
	...
	var password = $( "#password" ),
		...
		last = $( "#last" ),
		alias = $( "#alias" ),
		allFields = $( [] ).add( password ).add( password2 ).add( first ).add( last ).add( alias ),
		tips = $( ".validateTips" );
	...
	$( "#dialog-form" ).dialog({
		...
		buttons: {
			"Create an Account": function() {
				...
				if ( bValid ) {
					$.get("admin_submit.php?p=users&method=create", { first: first.val(), last: last.val(), password: password.val(), password2: password2.val(), alias: alias.val() },
function(data) {
...
$(function() {
	...
	var username = $( "#m_username" ),
		...
		last = $( "#m_last" ),
		alias = $( "#m_alias" ),
		allFields = $( [] ).add( username ).add( password ).add( password2 ).add( first ).add( last ).add( alias ),
		tips = $( ".validateTips2" );
	...
	$( "#dialog-form2" ).dialog({
		...
		buttons: {
			"Modify an account": function() {
				...
				if ( bValid ) {
					$.get("admin_submit.php?p=users&method=modify", { username: username.val(), password: password.val(), password2: password2.val(), alias: alias.val() },
function(data) {
   ...
   else
   {
	...
	function Modify(user,first,last,alias)
	{
		$( "#m_username" ).val(user);
		$( "#m_first" ).val(first);
		$( "#m_last" ).val(last);
...
<div id="users-contain" class="ui-widget">
	<table id="users" class="ui-widget ui-widget-content">
		...
		if ($query == "")
		{
			$q = mysql_query("SELECT * FROM auth WHERE type LIKE '%cs%' LIMIT 0,10")
			  or die(mysql_error());
			...
			else
			{
				while($r = mysql_fetch_assoc($q))
				{
					...
					echo "<td>" . $r["type"] . "</td>";
					echo "<td>" . $r["access"] . "</td>";
					echo "<td><a onclick='Modify(\"$r[user]\",\"$r[first]\",\"$r[last]\",\"$r[alias]\")' style='cursor:pointer; text-decoration:underline;'>Modify</a></td>";
					if($r["status"] == "Enabled")
					{
