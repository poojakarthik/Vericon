	...
	$( "#dialog-form" ).dialog({
		...
		buttons: {
			"Submit": function() {
				...
				if ( bValid ) {
					$.get("user_submit.php?method=create", { first: first.val(), last: last.val(), password: password.val(), password2: password2.val(), alias: alias.val() },
					function(data) {
...
$(function() {
	...
	$( "#dialog-form2" ).dialog({
		...
		buttons: {
			"Submit": function() {
				$.get("user_submit.php?method=modify", { username: username.val(), alias: alias.val() },
				function(data) {
					if (data == "modified")
					{
