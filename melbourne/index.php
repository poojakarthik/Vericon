...
<head>
<title>VeriCon :: Melbourne Sales</title>
...
while ($agent = mysql_fetch_assoc($q))
{
	$today = date("Y-m-d");
	$week = date("W");
	$q1 = mysql_query("SELECT (SELECT COUNT(*) FROM sales_customers WHERE agent = '$agent[user]' AND DATE(timestamp) = '$today'),(SELECT COUNT(*) FROM sales_customers WHERE agent = '$agent[user]' AND WEEK(timestamp) = '$week'),(SELECT COUNT(*) FROM sales_customers WHERE agent = '$agent[user]')") or die(mysql_error());
