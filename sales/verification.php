					...
					function(data) {
						if (data.substring(0,5) == "valid")
						{
							...
							$( "#display" ).hide('blind', '' , 'slow', function() {
								$( "#display" ).load('verification_dash.php?id=' + id, function() {
									$( "#packages" ).load('../tpv/packages.php?id=' + id, function() {
	...
	$( "#dialog-form2" ).dialog({
		...
		buttons: {
			"Add Package": function() {
				...
				{
					$.get("verification_submit.php?method=add", { id: id.val(), cli: cli.val(), plan: plan.val() },
					function(data) {
						if (data == "added")
						{
							$( "#packages" ).load('../tpv/packages.php?id=' + id.val());
...
{
	$( "#plan" ).val("");
	$( "#plan" ).load("../tpv/plans.php?id=" + $( "#id" ).val() + "&type=" + $( "#sale_type" ).val() + "&cli=" + $('#cli').val());
}
</script>
	...
	$( "#dialog-form3" ).dialog({
		...
		buttons: {
			"Edit Package": function() {
				...
				{
					$.get("verification_submit.php?method=edit", { id: id.val(), cli: cli.val(), plan: plan.val(), cli2: cli2.val() },
					function(data) {
						if (data == "editted")
						{
							$( "#packages" ).load('../tpv/packages.php?id=' + id.val());
...
{
	$( "#edit_cli" ).val(cli);
	$( "#edit_plan" ).load("../tpv/plans.php?id=" + $( "#id" ).val() + "&type=" + $( "#sale_type" ).val() + "&cli=" + $('#edit_cli').val(),
	function() {
		$( "#edit_plan" ).val(plan);
	});
...
function LoadScript()
{
	...
	$.get("verification_submit.php?method=script_check", { id: id.val()},
	function(data) {
		if (data == "valid")
		{
			$( "#display" ).hide('blind', '' , 'slow', function() {
				$( "#display" ).load('verification_script.php?id=' + id.val() + '&user=<?php echo $ac["user"]; ?>', function() {
					$( "#display" ).show('blind', '' , 'slow');
				});
			});
		}
	...
	$( "#dialog-form4" ).dialog({
		...
		buttons: {
			"Submit": function() {
				...
				$.get("verification_submit.php?method=cancel", { id: id.val(), verifier: verifier.val(), dialled: dialled.val(), status: status.val(), note: note.val() }, function(data) {
					if (data == "done")
					{
...
function Submit()
{
	...
	$.get("verification_submit.php?method=submit", { id: id.val(), verifier: verifier.val(), dialled: dialled.val(), note: note.val() },
	function(data) {
		if (data == "done")
		{
			$( "#display" ).hide('blind', '', 'slow', function() {
...
<div id="display">
<script>
$( "#display" ).load('verification_display.php?user=<?php echo $ac["user"]; ?>', function() {
	$( "#display" ).show('blind', '' , 'slow');
});
</script>
</div>
