...
if ($query == "")
{
	...
	$check = mysql_query("SELECT * FROM vericon.auth WHERE type LIKE '%Welcome%'") or die(mysql_error());
	...
	{
		$st = $_GET["page"]*15;
		$q = mysql_query("SELECT * FROM vericon.auth WHERE type LIKE '%Welcome%' ORDER BY user ASC LIMIT $st , 15") or die(mysql_error());
