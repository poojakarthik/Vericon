	...
	$( "#dialog-form2" ).dialog({
		...
		buttons: {
			"Add Package": function() {
				...
				{
					$.get("details_submit.php?method=add_au", { id: id, cli: cli.val(), plan: plan.val() },
					function(data) {
						if (data == "added")
						{
							$( "#packages" ).load('packages_au.php?id=' + id);
...
{
	$( "#plan" ).val("");
	$( "#plan" ).load("plans_au.php?id=<?php echo $_GET["id"]; ?>&type=" + $( "#sale_type" ).val() + "&cli=" + $('#cli').val());
}
</script>
	...
	$( "#dialog-form3" ).dialog({
		...
		buttons: {
			"Edit Package": function() {
				...
				{
					$.get("details_submit.php?method=edit_au", { id: id, cli: cli.val(), plan: plan.val(), cli2: cli2.val() },
					function(data) {
						if (data == "editted")
						{
							$( "#packages" ).load('packages_au.php?id=' + id);
...
{
	$( "#edit_cli" ).val(cli);
	$( "#edit_plan" ).load("plans_au.php?id=<?php echo $_GET["id"]; ?>&type=" + $( "#sale_type" ).val() + "&cli=" + $('#edit_cli').val(), function() {
		$( "#edit_plan" ).val(plan);
	});
...
{
	$( "#edit_plan" ).load("plans_au.php?id=<?php echo $_GET["id"]; ?>&type=" + $( "#sale_type" ).val() + "&cli=" + $('#edit_cli').val());
}
</script>
